...
**카카오가 이메일을 주지 않으면** 회원번호로 자리표시자를 만든다(`kakao-{id}@no-email.invalid`). 계정의 기본키 노릇을 하는 `email` 이 비어 있으면 계정 생성 자체가 실패하기 때문이다. 같은 사람은 늘 같은 값이 나오므로 재로그인해도 계정이 갈라지지 않고, 실제 주소가 아님이 도메인에 드러난다. 이 계정은 `emailVerified` 가 false 로 남아 관리자가 될 수 없다.
