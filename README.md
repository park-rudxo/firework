...
로그인 자체는 어느 프로바이더로도 되고, 프로젝트를 올리려면 GitHub 연결이 필요하다 — 그 계정으로 저장소 소유권까지 확인한다.

### 닉네임은 자기소개가 아니라 소속 표기다

가입한 사람은 예외 없이 **`기수_지역_반_이름`** 형식의 닉네임을 갖는다. `13_구미_1반_박경도` 처럼.
...
프로젝트를 올린 사람과 일정을 적은 사람이 누구인지 한 줄로 드러나야 하고, 같은 반끼리 서로를 찾을 수 있어야 한다. "박경도" 만으로는 그게 안 되고 동명이인을 구별할 방법도 없다.

**소셜 로그인만 있어서 가입 폼이 없다.** 프로바이더가 준 이름(`박경도`, `kdpark`)이 그대로 들어오므로, 첫 로그인 뒤 `/nickname` 을 한 번 세워 여기서 받는다. 형식을 갖추지 못한 사람은 로그인·이메일 확인·정책 문서를 빼고 어디로 가든 이 화면으로 돌아온다. 이미 가입한 사람도 같은 길을 지나므로 결국 전원이 같은 형식이 된다.
