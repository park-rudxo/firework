...
| 필요 인증 | Google / Kakao / Naver / GitHub **아무거나 하나** | 이메일 확인 (+ **저장소를 붙일 때만** GitHub 연결) |

외부인을 막지 않는다. 프로젝트를 올리는 데 필요한 것은 이메일 확인까지다.

**GitHub 연결은 저장소를 붙일 때만 요구한다.** 공유할 만한 결과물이 전부 공개 저장소를 가진 것은 아니기 때문이다 — 배포된 웹서비스, 스토어에 올린 앱, 사내 코드로 만든 것도 같은 자리에 선다. 그래서 프로젝트는 **저장소 주소와 서비스 주소 중 최소 하나**만 있으면 등록된다. 저장소를 넣은 경우에만 스타·언어·README 스냅샷을 가져오고, 연결된 GitHub 계정으로 소유권을 확인해 배지를 붙인다. 둘 다 비면 방문자가 써볼 방법이 없으므로 그때만 등록을 막는다.

**이메일 확인**은 소셜 로그인 뒤 한 번 거친다. 다만 프로바이더가 이미 검증해준 경우에는 그대로 통과한다 — Google 은 `email_verified` 클레임, GitHub 은 이메일 API 의 `verified`, Kakao 는 `is_email_verified` 를 주기 때문이다. **네이버는 Better Auth 가 `emailVerified` 를 항상 `false` 로 주므로** 사실상 네이버로 가입한 사람만 코드를 한 번 입력한다. 이미 확인된 것을 또 확인시키는 건 마찰만 늘린다.

확인이 필요한 곳은 **설문 응답(=추첨 응모)과 프로젝트 등록**뿐이다. 둘러보기·좋아요까지 막으면 얻는 것 없이 진입만 막힌다.

**약관과 개인정보처리방침**은 `/about/terms`, `/about/privacy` 에 있고 푸터와 로그인 화면에서 닿는다. 형식을 갖추려고 둔 것이 아니라 카카오 비즈 앱 전환과 네이버 로그인 검수에서 실제로 확인하는 항목이고, 익명 설문과 경품 추첨처럼 분쟁 소지가 뚜렷한 기능의 처리 기준을 적어둔 곳이다. 문의처는 `CONTACT_EMAIL` 로 넣는다(비우면 저장소 이슈 링크를 대신 보여준다).
...
**데모 데이터는 실서비스를 열기 전에 `npm run db:purge` 로 지운다.** 시드는 화면을 확인하려고 넣는 것이라 그대로 배포하면 "모아모아" 같은 가짜 프로젝트가 홈에 그대로 뜬다. `db:purge` 는 전체 삭제가 아니라 시드가 만든 행(`prisma/seed-manifest.ts` 의 표식이 붙은 것)만 골라 지우므로, 이미 진짜 사용자가 들어온 뒤에 돌려도 안전하다. 반대로 `db:seed` 는 기존 데이터를 전부 지우고 시작하기 때문에 **로컬이 아닌 데이터베이스를 향하면 실행을 거부한다**(정말 필요하면 `SEED_FORCE=1`).
...
npm run db:seed     # 개발용 시드 데이터 (로컬 DB 에서만)
npm run db:purge    # 시드가 만든 데모 데이터만 삭제 — 배포 전에 실행
