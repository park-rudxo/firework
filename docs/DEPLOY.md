...
- **`RESEND_API_KEY`** (선택) — https://resend.com/api-keys (무료 3,000통/월).
  이메일 확인은 게이트가 아니라서 없어도 로그인·설문·등록은 그대로 된다. 다만
  코드가 사용자 화면 대신 **서버 로그**로 가고, **관리자 승격은 확인된 이메일에만
  적용**되므로 Google·GitHub 이 아닌 계정을 관리자로 만들려면 필요하다.
  도메인을 붙이기 전에는 `EMAIL_FROM` 을 비워두면 Resend 테스트 발신 주소를 쓴다.
