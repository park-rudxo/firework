...
if (available.kakao) {
  socialProviders.kakao = {
     ...
     * 이메일은 아예 요청하지 않는다.
     *
     * 기본 스코프에 account_email 이 들어 있는데, 이 동의항목은 비즈 앱으로 전환해야
     * 열린다. 열리지 않은 앱에서 요청하면 동의 화면이 아니라 오류로 끝난다.
     * 닉네임·프로필 사진은 설정에서 바로 켤 수 있으므로 그 둘만 받는다.
     *
     * 이메일을 안 받으면 아래 mapProfileToUser 가 자리표시자를 만든다.
     */
    disableDefaultScope: true,
    scope: ["profile_nickname", "profile_image"],
    /**
     * 그래서 카카오 계정에는 이메일이 없다.
     *
     * 계정의 기본키 노릇을 하는 email 컬럼은 비어 있을 수 없어서, 그대로 두면
     * 카카오 로그인이 계정 생성 단계에서 통째로 실패한다. 회원번호로 자리표시자를
     * 만든다 — 같은 사람은 늘 같은 값이 나오므로 재로그인해도 계정이 갈라지지 않고,
     * 실제로 있는 주소가 아님이 도메인에 드러나 있다(RFC 2606 이 예약해둔 .invalid).
     *
     * 나중에 비즈 앱으로 전환해 이메일을 받게 되면 그 값을 그대로 쓴다.
     *
