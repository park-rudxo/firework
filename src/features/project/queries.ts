...
export async function listProjects({
  ...
  // 홈의 "이번 주 인기"는 여러 반응을 가중 합산하는데, 그 섹션의 "더 보기"가
  // 데려오는 이 목록은 좋아요 수만 봤다. 같은 이름으로 다른 순서를 보여주면
  // 사용자는 목록이 틀렸다고 여긴다. 정렬 기준은 한 곳(scoreProjects)에서만 온다.
 ...
 * 인기 점수 순 목록.
 *
 * 점수는 SQL 한 방으로 나오지 않으므로(반응 테이블 네 곳을 합산한다) 후보를 먼저
 ...
 * 내 프로젝트.
 *
 * 등록자인 것과 팀원으로 올라온 것을 함께 보여준다. ProjectMember 에 역할을
 * 만들어두고 목록은 ownerId 로만 뽑으면, 공동 관리자로 초대받은 사람은 자기
 * 대시보드에서 그 프로젝트를 아예 찾을 수 없다.
 */
