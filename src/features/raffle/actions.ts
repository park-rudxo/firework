...
export async function createRaffle(
  ...
  if (!(await canManageProject(project, viewer.id))) {
    return { error: "이 프로젝트의 관리 팀만 추첨을 열 수 있습니다." };
  }
...
export async function drawRaffle(raffleId: string): Promise<RaffleState> {
  ...
  if (!(await canManageProject(raffle.project, viewer.id))) return { error: "권한이 없습니다." };
...
export async function markPrizeDelivered(winnerId: string): Promise<RaffleState> {
  ...
  if (!winner) return { error: "당첨 정보를 찾을 수 없습니다." };
  if (!(await canManageProject(winner.raffle.project, viewer.id))) return { error: "권한이 없습니다." };
